<template>
    <div>
        <div v-if="this.$page.props.status.success && show"
             class="mb-8 flex items-center justify-between bg-green-500 rounded max-w-3xl">
            <div class="flex items-center">
                <svg class="ml-4 mr-2 flex-shrink-0 w-4 h-4 fill-white"
                     viewBox="0 0 20 20"
                     xmlns="http://www.w3.org/2000/svg">
                    <polygon points="0 11 2 9 7 14 18 3 20 5 7 18"/>
                </svg>
                <div class="py-4 text-white text-sm font-medium">{{ $page.props.status.success }}</div>
            </div>
            <button class="group mr-2 p-2"
                    type="button"
                    @click="show = false">
        ...
        <div v-if="(this.$page.props.status.error || Object.keys(this.$page.props.errors).length > 0) && show"
            ...
            <div class="flex items-center">
                ...
                <div v-if="this.$page.props.status.error"
                     class="py-4 text-white text-sm font-medium">{{ $page.props.status.error }}
                </div>
                <div v-else
                     class="py-4 text-white text-sm font-medium">
                    <span v-if="Object.keys(this.$page.props.errors).length === 1">There is one form error.</span>
